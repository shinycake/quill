...
/// `addMessageReaction` (TDLib 1.8.67). Unigram `ReactionButton.OnClick`
/// uses `is_big` false and `update_recent_reactions` false for the compact
/// strip / chip. `reaction_type` is `reactionTypeEmoji` only in this slice.
...
/// `removeMessageReaction` (TDLib 1.8.67). Schema: a chosen reaction can
/// always be removed. Unigram chip click on `IsChosen` sends this.
...
mod tests {
    ...
    fn add_and_remove_message_reaction_shapes_match_1_8_67() {
        ...
        assert!(!add.contains("CANARY"));
        let remove = remove_message_reaction(RequestId(42), ChatId(11), MessageId(101), "❤");
