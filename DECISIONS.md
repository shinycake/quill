...
- **Screenshots:** real GPUI window on Linux xvfb + lavapipe, `docs/screenshots/synthetic-chat.png` (plus composer and unsupported-auth shots), connect surfaces via `quill --screenshot-demo`, Phase 1 `ready-chats` / `ready-chats-composer`, unread proof `ready-unread` / `ready-unread-read`, media receive `ready-media`, outgoing attach/send `ready-send-media`, global search `ready-search`, in-chat search `ready-search-in-chat`, reply `ready-reply`, edit/delete `ready-edit-delete`, forward `ready-forward`, and reactions `ready-reactions` (injected Ready + inbox/outbox/media/search/reply/edit/forward/reaction updates, no live Telegram). The stray “X” in an early capture was the X11 cursor, not a jump button. VoiceOver remains a macOS follow-up.
...
## Message reactions (Phase 1)

- **Official clients first:** tdesktop per-message react affordance is `HistoryView::Reactions::Button` + expandable strip (`history_view_reactions_button.h` / `history_view_reactions_strip.h`); counts and chosen chips render as `HistoryView::Reactions::InlineList` under/beside the bubble (`history_view_reactions.h`). Unigram compact menu is `ReactionsMenuFlyout` (up to 7 of `AvailableReactions` Top+Recent+Popular; expand is out of this slice). Chip click is Unigram `ReactionButton.OnClick`: `IsChosen` → `RemoveMessageReaction`, else `AddMessageReaction(chatId, messageId, type, isBig: false, updateRecent: false)`. Chosen chips use a filled/accent treatment (`IsChecked = IsChosen`) plus count (`TotalCount`); Unigram a11y uses “You reacted with”. Esc closes the strip first.
- **Default set:** official launch / tdesktop fallback eight (`👍 👎 ❤ 🔥 🥰 👏 😁 🤔`). Heart is schema `reactionTypeEmoji` U+2764 `❤`, not `❤️`+VS16. Full picker, custom emoji, paid, and `getMessageAvailableReactions` stay out.
- **Schema (1.8.67, not invented):** `addMessageReaction chat_id message_id reaction_type is_big update_recent_reactions = Ok`. `removeMessageReaction chat_id message_id reaction_type = Ok`. Incoming aggregation is `message.interaction_info` / `updateMessageInteractionInfo` → `messageInteractionInfo.reactions` (`messageReaction type total_count is_chosen`). Only `reactionTypeEmoji` is typed; custom/paid are skipped.
- **UX:** Ready history rows get **React** (already-sent only). It opens a **React** overlay with the default eight; picking a chosen emoji toggles it off. Existing chips under the row show `{emoji} {count}` and `{emoji} {count} · you` when `is_chosen`. Chip click uses the same add/remove toggle. UI updates only from `updateMessageInteractionInfo` (demo injects that update). Esc: reaction picker, then forward picker, delete confirm, in-chat search, sidebar search, reply, edit, selection / forward success.
- **Out of this slice:** custom/paid reactions, `getMessageAvailableReactions` / `updateActiveEmojiReactions` live lists, big animation, who-reacted list, tags, channels/bots, voice/video, App Store, live Telegram in CI.
