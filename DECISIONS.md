...
- **Schema (1.8.67, not invented):** `searchRecentlyFoundChats` (`query`, `type_filter` null, `limit` 50) and `searchChats` (`query`, `type_filter` null, `limit` 20) return `chats`. `searchMessages` (`chat_list` null = all lists; schema only Main/Archive; `offset` `""`; `limit` 20; `filter` / `chat_type_filter` null; `min_date`/`max_date` 0) returns `foundMessages`. Select: `addRecentlyFoundChat` then existing `openChat`. Not used this slice: `searchPublicChat`, `searchChatsOnServer`, message-result pagination.
...
- **Official clients first:** tdesktop `Command::Search` (Ctrl/Cmd+F) with history focused is `HistoryWidget::searchInChat` → `HistoryView::ComposeSearch` (`history_view_compose_search.cpp`). The find bar has a query field (TopBar) plus hit counter / next / prev (BottomBar: `lng_search_messages_n_of_amount` / `lng_search_messages_none`). Esc / cancel closes the bar. F3 / Shift+F3 (and macOS Cmd+G / Cmd+Shift+G) walk hits. Typing debounces with `AutoSearchTimeout` (900 ms) via `_searchTimer.callOnce`. Empty query does not search. Unigram (TDLib-shaped) is `ChatSearchViewModel` + `SearchChatMessagesCollection` → `searchChatMessages`; selecting a hit calls `Dialog.LoadMessageSliceAsync` (jump in the same history).
- **Schema (1.8.67, not invented):** `searchChatMessages` (`chat_id`; `topic_id` null; `query`; `sender_id` null; `from_message_id` 0; `offset` 0; `limit` 50 = tdesktop `SearchPerPage`; `filter` null) returns `foundChatMessages` (`total_count`, `messages`, `next_from_message_id`). First page uses schema “from the last message”. Cmd/Ctrl+F with no open chat is a no-op and does **not** steal Cmd/Ctrl+K sidebar search.
- **UX:** Find bar above the open history. Hit count `n of amount` or “No messages found”. Next/prev walk the reverse-chrono hit list (newest is 1). Jump upserts the selected `foundChatMessages` row into the existing history map (same promote path as sidebar message hits) and highlights it. Switching chats closes the bar. Stale `@extra` is ignored.
- **Out of this slice:** filter-by-sender, calendar jump, migrated-history merge, secret-chat `searchSecretMessages`, pagination beyond the first page, VoiceOver.
