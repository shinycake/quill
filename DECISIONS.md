...
- **Official clients first:** tdesktop puts the field in the left chat column and binds **Ctrl/Cmd+F** (`Command::Search`). Unigram (TDLib-shaped) empty field is `searchRecentlyFoundChats`; typed query is `searchChats` then `searchMessages` with `chat_list` **null**; select sends `addRecentlyFoundChat`. Phase 1 matches that subset — not a greenfield palette. Skipped this slice (Unigram also does these): `getTopChats`, `searchContacts`, `searchChatsOnServer`, `searchPublicChats`.
...
- **UX:** Ready sidebar always shows a Search field (tdesktop). Cmd/Ctrl+F (official) or Cmd/Ctrl+K focuses it and loads **Recent**. Typing replaces the chat list with **Chats** + **Messages**. Esc/Clear empties to Recent, then closes (chat list returns). A message hit is upserted into history then opened — no new history pager. Stale `@extra` is ignored.
