...
- **Screenshots:** real GPUI window on Linux xvfb + lavapipe, `docs/screenshots/synthetic-chat.png` (plus composer and unsupported-auth shots), connect surfaces via `quill --screenshot-demo`, Phase 1 `ready-chats` / `ready-chats-composer`, unread proof `ready-unread` / `ready-unread-read`, media receive `ready-media`, outgoing attach/send `ready-send-media`, global search `ready-search`, in-chat search `ready-search-in-chat`, reply `ready-reply`, edit/delete `ready-edit-delete`, and forward `ready-forward` (injected Ready + inbox/outbox/media/search/reply/edit/forward updates, no live Telegram). The stray “X” in an early capture was the X11 cursor, not a jump button. VoiceOver remains a macOS follow-up.
...
## Forward messages (Phase 1)

- **Official clients first:** tdesktop context-menu / selection **Forward** opens a recipient picker (legacy `HistoryHider` / `history_widget.cpp` `forwardSelected`; modern `MainWidget::setForwardDraft` + `SectionShow::Way::Forward` then `FieldHeader::updateForwarding` in `history_view_compose_controls.cpp`). History rows paint origin via `HistoryView::Message::displayForwardedFrom` / `paintForwardedInfo` (`history_view_message.cpp`). Unigram (TDLib-shaped) `DialogViewModel.Messages` `ForwardMessages` opens `ChooseChatsViewModel`, which sends `ForwardMessages(chat.Id, topic, fromChatId, ids, options, sendCopy, removeCaption)` — default is a real forward (`send_copy` only when copy-mode / remove-captions). Esc / Cancel dismisses the picker (tdesktop `HistoryHider::startHide`). Clicking origin is out of this slice.
- **Schema (1.8.67, not invented):** `forwardMessages chat_id topic_id from_chat_id message_ids options send_copy remove_caption = Messages` (`topic_id` / `options` null; `message_ids` strictly increasing, at most 100). Incoming `message.forward_info` is `messageForwardInfo origin date source public_service_announcement_type` with `MessageOrigin` = `messageOriginUser` / `HiddenUser` / `Chat` / `Channel`. Live gate is `messageProperties.can_be_forwarded` (true forward) or `can_be_copied` (`send_copy: true` / `inputMessageForwarded` copy options). This slice uses the documented default `can_be_forwarded + can_be_copied` (same style as own-outgoing for edit/delete) until `getMessageProperties` is wired. Channels/bots stay out (sponsored-content).
- **UX:** Ready history rows get a **Forward** action (single message). Picker lists already-loaded **supported** chats (`ordered_chats` / same local title filter as sidebar search) — no second search stack. Sidebar / search chat click while the picker is open also picks the destination. Confirm sends `forwardMessages` and opens the dest (tdesktop `Way::Forward`). Rows with `forward_info` show a compact **Forwarded from {origin}** header. Esc: forward picker, then delete confirm, in-chat search, sidebar search, reply, then cancel edit. Do not steal ⌘/Ctrl+↑ Load older.
- **Out of this slice:** multi-select albums, `getMessageProperties` live fetch, `getTopChats` / `topChatCategoryForwardChats`, click-origin jump, caption-strip copy UI, secret chats, stories, reactions, voice/video, App Store, live Telegram in CI.
