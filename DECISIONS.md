...
- **Screenshots:** real GPUI window on Linux xvfb + lavapipe, `docs/screenshots/synthetic-chat.png` (plus composer and unsupported-auth shots), connect surfaces via `quill --screenshot-demo`, Phase 1 `ready-chats` / `ready-chats-composer`, unread proof `ready-unread` / `ready-unread-read`, media receive `ready-media`, outgoing attach/send `ready-send-media`, global search `ready-search`, in-chat search `ready-search-in-chat`, reply `ready-reply`, edit `ready-edit`, and delete `ready-delete` (injected Ready + inbox/outbox/media/search/reply/edit/delete updates, no live Telegram). The stray “X” in an early capture was the X11 cursor, not a jump button. VoiceOver remains a macOS follow-up.
...
- **Out of this slice:** sender/tag filters, calendar jump, secret-chat `searchSecretMessages`, edit/delete/reactions (replies are the next slice).
...
- **Official clients first:** tdesktop `ComposeControls::editMessage` / `cancelEditMessage` and `FieldHeader::editMessage` fill the field with the message text and show an edit header; Escape / cancel drops the header and restores the pre-edit draft (`history_view_compose_controls.h`). Per-message **Edit** is the pick-a-message action (same row pattern as Reply). tdesktop `DeleteMessagesBox` (`boxes/delete_messages_box.cpp`) confirms delete and offers revoke: private chats “Delete for [peer]”, groups “Delete for everyone”. Quill uses explicit **Delete for me** / **Delete for everyone** buttons gated by TDLib flags (same choice as official mobile; checkbox would hide the second path on this Ready shell). History bubbles show **edited** when `message.edit_date > 0` (tdesktop `HistoryItem` edit date).
- **Unigram / TDLib:** capabilities are `getMessageProperties` → `messageProperties` (`can_be_edited`, `can_be_deleted_only_for_self`, `can_be_deleted_for_all_users`). Edit is `editMessageText` (`reply_markup` null, `inputMessageText`). Delete is `deleteMessages` (`revoke` = for everyone). Content vs date: `updateMessageContent` then `updateMessageEdited`.
- **Schema (1.8.67, not invented):** `editMessageText chat_id message_id reply_markup input_message_content = Message`. `deleteMessages chat_id message_ids revoke = Ok` (`revoke` always true for supergroups, channels, secret chats). `getMessageProperties chat_id message_id = MessageProperties`. `updateMessageContent` / `updateMessageEdited` (`edit_date`).
- **UX:** own text rows with `can_be_edited` get **Edit**; own rows with delete flags get **Delete**. Composer shows **Editing message** + Cancel. Esc: in-chat search, sidebar search, reply, delete confirm, then cancel edit (restore stash). History updates from the TDLib updates (demo injects the same JSON).
- **Out of this slice:** edit caption/media, incoming delete-for-me, forward, reactions, voice/video, quote-from-selection, channels/bots sponsored, App Store.
