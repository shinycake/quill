...
- [x] Self-destructing messages (Phase B3): per-media `self_destruct_type` on `inputMessagePhoto` / `inputMessageVideo` — **private (1:1 cloud) chats only**, verified against pinned TDLib `d1085f9` (400 "Messages can self-destruct only in private chats" unless `DialogType::User`; secret chats self-destruct through the chat-level timer mechanism instead (see Phase B4 below); composer ⏱ picker (Off / 5s / 30s / 1m / View once — the server caps the timer at 60s) gated to private chats with photo/video pending, driver strips the choice elsewhere; receive parses `message.self_destruct_type` / `self_destruct_in` with live ⏱ countdown badges and a 1s render tick; rows disappear via `updateDeleteMessages`; plain text can't carry self-destruction (`inputMessageText` has no such field — not faked) (injected/replay; live needs tdjson)
- [x] 1-on-1 call signaling + call UI (Phase C1): audio-only `createCall` / `acceptCall` / `discardCall` / `sendCallRating` with the full `updateCall` state machine (Pending → ExchangingKeys → Ready → HangingUp → Discarded / Error); `updateNewCallSignalingData` queued honestly (32-chunk cap) but never consumed — **TDLib carries no audio/video transport, and the UI says so on every live-call card and the end screen (transport deferred to Phase C2 / libtgvoip; `callProtocol` advertises no UDP)**; incoming (Accept / Decline) / outgoing (Cancel) / connected (duration, Hang up) overlay, end screen with reason line + 1–5 rating prompt on `need_rating`, busy-decline for a second incoming call, surfaced request errors (TDLib message text never stored); video, group calls/voice chats, and debug/log upload are out of slice (injected/replay; live needs tdjson)
- [x] Chat-level auto-delete / self-destruct timer (Phase B4): `setChatMessageAutoDeleteTime` + `chat.message_auto_delete_time` (schema 1.8.67 :13454 / :3616+:3627 — secret chats: arbitrary seconds from view; others: 0–365 day-multiples from send) — secret-chat timers fully shipped: header ⏱ picker (Off / 5s / 30s / 1m / 1h / 1d / 1w), timer status line, `messageChatSetMessageAutoDeleteTime` service rows, per-message `auto_delete_in` countdown chips on the shared 1s tick, driver validation, rows leave via `updateDeleteMessages`; regular-chat receive/state/driver layers done, picker UI deferred until `change_info` rights gating is known (immediate follow-up) (injected/replay; live needs tdjson)
