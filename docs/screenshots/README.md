...
| `ready-audio-send.png` | Composer **Attach audio** chip (`demo-track.mp3`) plus an own-sent music file (**Night Drive** / Ada Lovelace / **Pause**, album cover, caption). Injected send path; no live Telegram. Driven by `quill --screenshot-demo ready-audio-send`. |
...
| `fixtures/demo-track.mp3` | Short MP3 with title, performer, and an embedded cover, used as the picked music file for audio send (not a screenshot). |
...
#   cargo run --features ui -- --screenshot-demo ready-audio-send docs/screenshots
