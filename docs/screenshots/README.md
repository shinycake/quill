...
| `ready-notification-sound.png` | **Parity slice 6** Notification settings: the header **Notifications** panel for Demo chat A (status **Unmuted · Sound: Ding · Previews: on**, mute presets 1 hour / 8 hours / 2 days / Forever, **Show message preview** toggle, and the sound picker expanded: **Default** / **None** / **✓ Ding (2s)** / **Chime (3s)** each with a ▶ preview button, plus a **Defaults for all chats…** link to the scope-defaults dialog). Injected `notificationSounds` / `updateChatNotificationSettings` / `scopeNotificationSettings`, no live Telegram. Driven by `quill --screenshot-demo ready-notification-sound` (taller window via `QUILL_DEMO_WINDOW_SIZE=1200x1150`). |
| `ready-slow-mode.png` | **Phase A1** Slow-mode enforcement: "Slow-mode demo group" (id 17) open as a plain member with `slow_mode_delay: 30` / `slow_mode_delay_expires_in: 25.0` — the composer shows the **Slow mode · wait 22s** countdown banner ("sending is paused until the timer expires") and the typed send is blocked (status note). Injected `supergroupFullInfo` / `updateSupergroup`, no live Telegram. Driven by `quill --screenshot-demo ready-slow-mode`. |
| `ready-secret-chat.png` | **Phase B1** Secret chat lifecycle: the Ready secret chat (id 41) with **Zed** open — the chat-list row carries the blue 🔒 badge, the header shows **Close secret chat** (with a confirm banner before sending `closeSecretChat`), three injected E2E messages, and the composer is live with typed text. Injected `updateSecretChat` (Ready, before `updateNewChat`) / `chatTypeSecret` / `updateNewMessage`, no live Telegram. Driven by `quill --screenshot-demo ready-secret-chat`. |
| `ready-key-verification.png` | **Phase B2** Secret chat key verification: the Ready secret chat with **Zed** open and his **Contact info** panel showing the **Encryption key** section — the 12×12 fingerprint grid from the deterministic 36-byte `key_hash` fixture (schema 1.8.67 :2812–2813) plus the verification copy ("If this image matches the one on your contact's device, your conversation is secure."). Injected `updateSecretChat` (Ready, real `key_hash`) / `chatTypeSecret`, no live Telegram. Driven by `quill --screenshot-demo ready-key-verification`. |
| `ready-self-destruct.png` | **Phase B3** Self-destructing media: a Ready **private** (1:1 cloud) chat with **Zed** — an incoming photo with a live **⏱ 44s left** countdown badge (`messageSelfDestructTypeTimer`, `self_destruct_in` decaying locally), an outgoing **⏱ view once** photo (`messageSelfDestructTypeImmediately`), and the composer's **⏱ 30s** timer picker next to the attachment chip (Off → 5s → 30s → 1m → View once). Private chat — not a secret chat — because TDLib only accepts per-media `self_destruct_type` in `chatTypePrivate` chats (schema 1.8.67 :6115/:6126, "private chats only"). Injected `updateNewMessage` / `chatTypePrivate`, no live Telegram. Driven by `quill --screenshot-demo ready-self-destruct`. |
...
| `ready-video-playback.png` | **Parity slice 5** Fullscreen **media viewer** open on a 12 s demo video (message 204, file 96): the video frame renders in-viewer (ffmpeg test pattern, not the thumbnail), with **❚❚ Pause**, elapsed `0:07 / 0:12`, zoom controls (`− 100% + Reset`), caption "Demo clip", and Prev/Next navigation. Genuine animated playback captured mid-animation: frames are extracted synchronously at 8 fps (96 frames), pre-decoded into in-memory image handles (see DECISIONS.md), and each refresh tick renders the frame for the current clock position — no fixed-frame special case. ffplay skipped in the demo. Fixture: `docs/screenshots/fixtures/demo-clip-12s.mp4` (320×180, 30 fps, 12 s). Driven by `quill --screenshot-demo ready-video-playback`. |
